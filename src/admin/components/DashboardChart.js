          ...
          {data.map((item) => (
            <Box key={item[labelKey]}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                <Typography variant="body2" sx={{ fontWeight: 500 }}>{item[labelKey]}</Typography>
                <Typography variant="body2" color="text.secondary">{item[valueKey].toLocaleString()}</Typography>
              </Box>
              <LinearProgress
                variant="determinate"
                value={(item[valueKey] / max) * 100}
                sx={{ height: 8, borderRadius: 4, bgcolor: 'rgba(0,0,0,0.06)', '& .MuiLinearProgress-bar': { borderRadius: 4, bgcolor: color || 'primary.main' } }}
              />
            </Box>
          ))}
        </Box>
...
export function RankedItemsCard({ title, data, subtitle, valueKey = 'orders', valueLabel }) {
  const { t } = useTranslation();
  const maxVal = data.length > 0 ? data[0][valueKey] : 1;
  const label = valueLabel || t('admin.dashboard.chart.orders');
  return (
    <Card sx={{ height: '100%' }}>
      <CardContent sx={{ p: 3 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {data.map((item, index) => (
            <Box key={item.name} sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <Typography variant="body2" sx={{ width: 24, height: 24, borderRadius: '50%', bgcolor: index < 3 ? 'primary.main' : 'grey.300', color: index < 3 ? 'white' : 'text.secondary', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 600, flexShrink: 0 }}>
                {index + 1}
              </Typography>
              <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexShrink: 0 }}>
                    <Typography variant="body2" color="text.secondary">{item[valueKey]} {label}</Typography>
                    {item.trend != null && (
                      <Chip
                        size="small"
                ...
                <LinearProgress variant="determinate" value={(item[valueKey] / maxVal) * 100} sx={{ height: 6, borderRadius: 3, bgcolor: 'rgba(0,0,0,0.06)', '& .MuiLinearProgress-bar': { borderRadius: 3, background: 'linear-gradient(135deg, #FF6B4A, #FF8F73)' } }} />
              </Box>
            </Box>
          ))}
        </Box>
  ...
  return (
    <Card sx={{ height: '100%' }}>
      <CardContent sx={{ p: 3 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {data.map((item) => (
            <Box key={item.name || item.title} sx={{ p: 2, borderRadius: 2, border: '1px solid', borderColor: 'divider', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
              <Typography variant="body2" sx={{ fontWeight: 600 }}>{item.name || item.title}</Typography>
              <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                ...
                <Box sx={{ textAlign: 'center' }}>
                  ...
                  <Typography variant="body2" sx={{ fontWeight: 600, color: 'success.main' }}>{item.conversionRate}%</Typography>
                </Box>
                {item.revenue != null && (
                  <Box sx={{ textAlign: 'center' }}>
                    <Typography variant="caption" color="text.secondary">{t('admin.dashboard.chart.revenue')}</Typography>
                    <Typography variant="body2" sx={{ fontWeight: 600 }}>${item.revenue.toLocaleString()}</Typography>
                  </Box>
                )}
              </Box>
            </Box>
...
export function AlertCard({ title, items, severity }) {
  const { t } = useTranslation();
  const colors = {
    warning: { bg: 'rgba(255, 184, 48, 0.08)', border: 'rgba(255, 184, 48, 0.3)', text: '#FFB830' },
    error: { bg: 'rgba(229, 62, 62, 0.08)', border: 'rgba(229, 62, 62, 0.3)', text: '#E53E3E' },
    info: { bg: 'rgba(255, 107, 74, 0.08)', border: 'rgba(255, 107, 74, 0.3)', text: '#FF6B4A' },
  };
  const c = colors[severity] || colors.warning;
  return (
    <Card sx={{ height: '100%', border: '1px solid', borderColor: c.border, bgcolor: c.bg }}>
      <CardContent sx={{ p: 3 }}>
        <Typography variant="h6" sx={{ mb: 2, fontWeight: 600, color: c.text }}>{title}</Typography>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          {items.map((item) => (
            <Box key={item.name} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <Chip label={`${item.orders} ${t('admin.dashboard.chart.orders').toLowerCase()}`} size="small" variant="outlined" />
            </Box>
          ))}
        </Box>
      </CardContent>
    ...
    <Card sx={{ height: '100%' }}>
      <CardContent sx={{ p: 3 }}>
        ...
        <Box sx={{ display: 'flex', borderRadius: 4, overflow: 'hidden', height: 24, mb: 2 }}>
          {data.map((item) => (
            <Box key={item.status} sx={{ width: `${(item.count / total) * 100}%`, bgcolor: item.color, minWidth: item.count > 0 ? 4 : 0 }} />
          ))}
        </Box>
