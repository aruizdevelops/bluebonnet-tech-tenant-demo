...
import { Box, Skeleton, Typography, Toolbar, useMediaQuery, useTheme } from '@mui/material';
import { TenantProvider, CoreThemeProvider, TopBar, Sidebar, DRAWER_WIDTH } from '@bluebonnet-tech/core';
...
function AdminShellInner({ children }) {
  ...
  useEffect(() => {
    ...
    checkAdminAuth().then((result) => {
      if (!cancelled) setAuthState({ checked: true, authenticated: result.authenticated });
    });
  ...
  if (!authState.authenticated) {
    return (
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
        <Typography variant="h5" color="error">{t('admin.common.accessDenied')}</Typography>
      </Box>
    );
  }
        ...
        <Sidebar
          ...
          sx={{
            ...
            '& .MuiListItemButton-root.Mui-selected': {
              ...
              '&:hover': { bgcolor: 'rgba(255, 107, 74, 0.18)' },
            },
          }}
        />
        <Box component="main" sx={{ flexGrow: 1, ml: { md: `${DRAWER_WIDTH}px` }, width: { md: `calc(100% - ${DRAWER_WIDTH}px)` } }}>
          <Toolbar />
          {children}
        </Box>
      </Box>
